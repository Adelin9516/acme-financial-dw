...
async function seed() {
  ...
  const nasdaq = await createDataSource({
    vendorName: "Nasdaq Data Link",
    apiEndpoint: "https://data.nasdaq.com/api/v3",
    description: "Nasdaq historical EOD data - free tier",
  });
  const bloomberg = await createDataSource({
    vendorName: "Bloomberg (Mock)",
    apiEndpoint: "https://bloomberg.com/api/mock",
    description: "Bloomberg mock data source for demo",
  });
  console.log(`  ✓ Nasdaq: ${nasdaq.id}`);
  ...
  for (const asset of createdAssets) {
    ...
    const dataPoints = series.map((p) => ({
      ...
      dataSourceId: nasdaq.id,
      timestamp: p.timestamp,
      metrics: p.metrics,
    }));

    // Insert in chunks of 400 (Firestore batch limit)
  ...
  console.log("\n✅ Seed complete!");
  console.log("\nSummary:");
  console.log(`  Data Sources: ${nasdaq.id}, ${bloomberg.id}`);
